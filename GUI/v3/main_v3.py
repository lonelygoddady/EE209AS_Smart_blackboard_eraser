...
class myApp(QtGui.QMainWindow, Project_GUI_v2.Ui_MainWindow):
    ...
    def paintEvent(self,pixm):
        global x1, x2, y1, y2, press_num, pix, size 
        if (press_num == 2):
           ...
           # scale_pix = pix.scaled(self.label_area.size(),QtCore.Qt.KeepAspectRatio,QtCore.Qt.FastTransformation) 
            painter.end()
            self.label_area.setPixmap(pix)    
    ...
    def update_content(self):
        ...
        pix = QtGui.QPixmap('sample.png')
        scale_pix = pix.scaled(self.label_area.size(),QtCore.Qt.KeepAspectRatio,QtCore.Qt.SmoothTransformation)       
